-- Patched floating renamer: upstream NvChad/ui prepareRename path can nil-index `result`.
local lsp = vim.lsp
local api = vim.api
  ...
  get_symbol_to_rename(function(to_rename)
    ...
    vim.fn.prompt_setcallback(buf, function(text)
      ...
      if #newName > 0 and newName ~= to_rename then
        local rename_clients = lsp.get_clients { bufnr = 0, method = "textDocument/rename" }
        local enc = (rename_clients[1] and rename_clients[1].offset_encoding) or "utf-8"
        local params = lsp.util.make_position_params(0, enc)
